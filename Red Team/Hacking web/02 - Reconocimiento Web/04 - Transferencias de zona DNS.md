...
> Las consultas DNS normales usan `UDP/53`, pero las transferencias de zona viajan por `TCP/53` porque el volumen de datos supera el tamaño de un datagrama UDP. Por eso un firewall puede permitir DNS UDP y bloquear `AXFR` simplemente filtrando TCP/53.
...
> La línea `XFR size: 50 records` y la cascada de registros confirman el vuelco. A partir de ahí tienes el inventario completo de la zona sin haber tocado ni un solo servidor web del objetivo.
