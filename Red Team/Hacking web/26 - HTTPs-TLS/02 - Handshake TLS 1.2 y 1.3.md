...
> - **Cripto post-cuántica** (actualizado 2026-08): el intercambio híbrido **`X25519MLKEM768`** ya no es una curiosidad — es **el grupo por defecto** de TLS 1.3 en Chrome (desde 131), Firefox (desde 135) y **OpenSSL 3.5+**, y está desplegado por defecto en Cloudflare, AWS, Azure y Microsoft 365. En el segundo trimestre de 2026 **supera el 50 % de las peticiones web**. Combina X25519 clásico con ML-KEM ([FIPS 203](https://csrc.nist.gov/pubs/fips/203/final), antes CRYSTALS-Kyber) en paralelo, así que basta con que uno de los dos aguante.
>   Lo relevante para un pentest: el ataque no es romper el cifrado hoy, es ***harvest now, decrypt later*** — capturar tráfico ahora para descifrarlo cuando haya un ordenador cuántico útil. Por eso, en un objetivo que maneje datos con vida útil larga (sanitarios, legales, secretos industriales), <mark style="background: #FF5582A6;">**la ausencia de intercambio híbrido sí es un hallazgo reportable**</mark>, aunque de severidad baja y con recomendación a medio plazo. Se comprueba mirando la extensión `supported_groups` del `ClientHello` y el grupo negociado en el `ServerHello`.
