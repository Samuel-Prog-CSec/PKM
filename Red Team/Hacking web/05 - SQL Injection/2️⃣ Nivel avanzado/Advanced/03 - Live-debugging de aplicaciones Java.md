...
La [[02 - Búsqueda de strings en el código|búsqueda estática]] localiza las consultas sospechosas, pero a veces hay que ver **en tiempo real** cómo se transforma nuestra entrada antes de llegar a la query —especialmente cuando hay filtros, normalización o concatenaciones complejas de por medio—. El *live-debugging* remoto permite pausar la ejecución, inspeccionar variables y seguir el flujo paso a paso. Es la fase de [[03 - Local testing - réplica del backend y depuración|local testing]] de un whitebox pentest aplicada a Java: el mismo razonamiento —confirmar con el depurador cómo se ve la entrada al llegar al `sink`— con JDWP en lugar del inspector de Node.
