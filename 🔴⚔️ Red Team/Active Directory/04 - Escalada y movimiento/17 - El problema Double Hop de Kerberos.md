...
> El double hop aparece en cuanto usas `evil-winrm` o `Enter-PSSession` y tratas de saltar más allá. Si un comando "debería funcionar" y da *Access Denied* desde una sesión remota, sospecha del double hop antes que de los permisos. Cuando el salto es además entre redes distintas, entra en juego el pivoting (módulo `007 - Pivoting y túneles`).
