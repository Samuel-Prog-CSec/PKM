...
- **Double-submit cookie**: el token viaja a la vez en una cookie y en un parámetro, y el backend solo comprueba que coincidan. El patrón cae si el atacante puede **leer o fijar** la cookie del token: con una CORS misconfig que permita leer respuestas, basta reflejar el valor de la cookie en el parámetro de la petición.
