...
Desde **MongoDB 8.0**, `$where` (junto a `$function`/`$accumulator`) está **deprecado**; el reemplazo recomendado es <mark style="background: #FF5582A6;">`$expr`</mark>, que compara campos con operadores de agregación **sin evaluar JavaScript**. Un matiz de rendimiento que no hay que vender de más: `$expr` solo aprovecha índices al comparar un campo contra una **constante**; comparar dos campos del mismo documento hace `COLLSCAN` igual que `$where` — la ventaja real ahí es evitar el motor JS, no ganar un índice. Aun así, deshabilita la evaluación de JS server-side si el proyecto no la necesita.

> [!info]+ Modernización: el ecosistema Node.js/Express
> HTB solo muestra PHP, pero la mayoría de NoSQLi vive hoy en Node. Ahí la defensa de referencia es **Mongoose con schemas tipados** (`username: String` rechaza objetos automáticamente por *casting*; Mongoose 8+ trae además `sanitizeFilter`), más validadores como `joi` o `express-validator`. <mark style="background: #FF5582A6;">Ojo con `express-mongo-sanitize`</mark>: era el middleware clásico (elimina claves con `$`/`.`), pero **está abandonado** (última release 2.2.0, ene 2022) y **roto en Express 5** — muta `req.query`, que ahora es un getter de solo lectura, así que lanza `TypeError` (modo estricto) o falla en silencio sin sanitizar nada. Si hace falta, usar el fork mantenido `@exortek/express-mongo-sanitize`. Regla de oro: **nunca pasar `req.body` crudo a un `find()`**.
