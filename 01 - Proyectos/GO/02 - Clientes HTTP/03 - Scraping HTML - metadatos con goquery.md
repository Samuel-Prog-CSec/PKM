...
Con esto cierras el Cap. 3: sabes construir clientes HTTP robustos (timeouts, context), diseñar clientes de API idiomáticos (functional options), hablar formatos binarios (MessagePack) y extraer datos donde no hay API (scraping + metadatos). El siguiente bloque le da la vuelta: en vez de consumir servicios, **montar los tuyos** — servidores HTTP para phishing, keylogging y C2 → [[Servidores HTTP.base|Servidores HTTP]] (Cap. 4).
